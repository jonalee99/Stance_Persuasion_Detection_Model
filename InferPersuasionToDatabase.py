...
documents = database.get_unrated_persuasion_documents()
...
# Load in the model
model = tf.keras.models.load_model(os.path.join("saved_models", "persuasion-hidden-128-lr-0.001-drop-0.3-batch-50"),
...
for i in range(math.ceil(len(documents)/batch_size)):
    ...
    for idx_, documentx in enumerate(document_batch):
        ...
        query = "UPDATE comments SET inferred_persuasion = {} WHERE ROWID == {}".format(int(predicted_result), int(documentx[0]))
        # print(query)
        # query = "UPDATE comments SET inferred_attitude = 0 WHERE ROWID == 5"
        cursor.execute(query)

        query = "UPDATE comments SET inferred_persuasion_weight = {} WHERE ROWID == {}".format(
